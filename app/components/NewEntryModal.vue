<script setup lang="ts">
import { ref, watch } from 'vue'
import { todayISO } from '~/utils/format'

const props = defineProps<{ open: boolean; title?: string }>()
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'save', payload: { name: string; dueDate: string; budgeted: number; actual: number }): void
}>()

const name = ref('')
const dueDate = ref(todayISO())
const budgeted = ref(0)
const actual = ref(0)

watch(
  () => props.open,
  (v) => {
    if (v) {
      name.value = ''
      dueDate.value = todayISO()
      budgeted.value = 0
      actual.value = 0
    }
  }
)

function save() {
  if (!name.value.trim()) return
  emit('save', {
    name: name.value.trim(),
    dueDate: dueDate.value,
    budgeted: Number(budgeted.value) || 0,
    actual: Number(actual.value) || 0,
  })
  emit('close')
}
</script>

<template>
  <div
    v-if="props.open"
    class="fixed inset-0 z-50 bg-slate-900/60 flex items-end sm:items-center justify-center"
    @click.self="emit('close')"
  >
    <div class="w-full max-w-[430px] bg-white rounded-t-2xl sm:rounded-2xl p-5 space-y-4">
      <div class="flex justify-between items-center">
        <h2 class="text-lg font-semibold">{{ props.title ?? 'New entry' }}</h2>
        <button
          type="button"
          class="min-h-[44px] min-w-[44px] rounded-full text-slate-500 text-2xl"
          aria-label="Close"
          @click="emit('close')"
        >
          ×
        </button>
      </div>

      <label class="block">
        <span class="text-sm text-slate-600">Name</span>
        <input
          v-model="name"
          type="text"
          class="mt-1 block w-full min-h-[44px] rounded-lg border border-slate-300 px-3"
        />
      </label>

      <label class="block">
        <span class="text-sm text-slate-600">Due date</span>
        <input
          v-model="dueDate"
          type="date"
          class="mt-1 block w-full min-h-[44px] rounded-lg border border-slate-300 px-3"
        />
      </label>

      <div class="grid grid-cols-2 gap-3">
        <label class="block">
          <span class="text-sm text-slate-600">Budgeted (€)</span>
          <input
            v-model.number="budgeted"
            type="number"
            step="0.01"
            inputmode="decimal"
            class="mt-1 block w-full min-h-[44px] rounded-lg border border-slate-300 px-3"
          />
        </label>
        <label class="block">
          <span class="text-sm text-slate-600">Actual (€)</span>
          <input
            v-model.number="actual"
            type="number"
            step="0.01"
            inputmode="decimal"
            class="mt-1 block w-full min-h-[44px] rounded-lg border border-slate-300 px-3"
          />
        </label>
      </div>

      <div class="flex gap-2 pt-2">
        <button
          type="button"
          class="flex-1 min-h-[44px] rounded-xl bg-slate-200 text-slate-700 font-medium"
          @click="emit('close')"
        >
          Cancel
        </button>
        <button
          type="button"
          class="flex-1 min-h-[44px] rounded-xl bg-emerald-600 text-white font-semibold"
          @click="save"
        >
          Add
        </button>
      </div>
    </div>
  </div>
</template>
